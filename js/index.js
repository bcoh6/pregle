window.addEventListener("load", function(){
		...
		// prev next X -> if()구문사용 존재유무판단
		// 하지만 전체에 다 있다면,, 하나하나if하기 힘듦
		
		// 캡처링 -> 부모를 뚫고 자식 클릭
		// 버블링 -> 자식을 뚫고 부모 클릭(캡처링 반대)
		...
		function rolling_Slide(_targetWrap, _view_ea_D, _view_ea_T, _view_ea_M){
			...
			// 스와이프(start)
				...
				// touchstart(mouse down), touchmove(mousemove), touchend(mouseup)
		...
		let rolling_Slide_01 = rolling_Slide(".bread", 4, 3, 2);
		let rolling_Slide_02 = rolling_Slide(".cake", 4, 3, 2);
		let rolling_Slide_03 = rolling_Slide(".cookie", 4, 3, 2);
		let rolling_Slide_04 = rolling_Slide(".review", 4, 3, 1);
